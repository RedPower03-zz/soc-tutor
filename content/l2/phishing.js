...
const items = [
  {
    ...
    choices: [
      ...
      'Return-Path, which bounces also use',
      'The top (most recent) Received header',
      'Message-ID, which tracks threads',
    ],
    answer: 'Reply-To (if present), otherwise From',
    misconceptions: {
      'Return-Path, which bounces also use': 'phish-from-trust',
    },
    explanation:
  ...
  },
  {
    ...
    snippet: AUTH_SPOOF,
    choices: [
      'SPF passed for the relay only; DMARC failed: From is spoofed',
      'SPF passed, so the email really came from Northwind Supply',
      'DMARC failed only because DKIM is missing, normal for suppliers',
      'Nothing: Reply-To addresses always differ from the From address',
    ],
    answer: 'SPF passed for the relay only; DMARC failed: From is spoofed',
    misconceptions: {
      'SPF passed, so the email really came from Northwind Supply': 'phish-dmarc-alignment',
      'Nothing: Reply-To addresses always differ from the From address': 'phish-from-trust',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Bottom to top: each server adds its line on top',
      'Top to bottom: the first line is where it started',
      'Any order: mail servers sort them alphabetically',
      'Only the top line matters; the rest are copies',
    ],
    answer: 'Bottom to top: each server adds its line on top',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'mailer.example signed it and the signed parts are unchanged',
      'The email is safe to open, because its signature is valid',
      'The person named in the From header personally wrote the email',
      'The email was encrypted end to end between both servers',
    ],
    answer: 'mailer.example signed it and the signed parts are unchanged',
    misconceptions: {
      'The email is safe to open, because its signature is valid': 'phish-auth-pass-safe',
      'The person named in the From header personally wrote the email': 'phish-from-trust',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      'corp.example',
      'login.corp.example',
      'owa.account-verify.example',
    ],
    answer: 'account-verify.example',
    misconceptions: {
      'corp.example': 'phish-url-domain',
      'login.corp.example': 'phish-url-domain',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Reset the password, revoke sessions, then check for attacker activity',
      'Nothing more is needed: the email is already deleted from the mailbox',
      'Tell the user to be more careful and to report links next time',
      'Block the sender\'s address at the mail gateway and close the ticket',
    ],
    answer: 'Reset the password, revoke sessions, then check for attacker activity',
    misconceptions: {
      'Nothing more is needed: the email is already deleted from the mailbox': 'phish-delete-enough',
      'Block the sender\'s address at the mail gateway and close the ticket': 'phish-delete-enough',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Compromised: an attacker is hiding payment replies (T1564.008)',
      'Emma is organising her inbox with a new rule after signing in',
      'A normal spam filter update pushed to her mailbox by IT',
      'The phishing attempt failed, so the sign-in was rejected',
    ],
    answer: 'Compromised: an attacker is hiding payment replies (T1564.008)',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Yes: aligned DKIM passing is enough for DMARC',
      'No: SPF failed, so DMARC must fail as well here',
      'No: DMARC needs both SPF and DKIM to pass',
      'DMARC is never checked on forwarded mail',
    ],
    answer: 'Yes: aligned DKIM passing is enough for DMARC',
    misconceptions: {
      'No: SPF failed, so DMARC must fail as well here': 'phish-dmarc-alignment',
  ...
  },
  {
    ...
    choices: [
      'Only that the lookalike domain itself authorised it',
      'It is genuinely from Northwind Supply\'s mail servers',
      'It is safe to pay, since all three checks passed',
      'The sending domain must be at least a year old',
    ],
    answer: 'Only that the lookalike domain itself authorised it',
    misconceptions: {
      'It is genuinely from Northwind Supply\'s mail servers': 'phish-auth-pass-safe',
      'It is safe to pay, since all three checks passed': 'phish-auth-pass-safe',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'To 203.0.113.50: the part before @ is just a username',
      'To portal.corp.example, the company\'s real login portal',
      'To corp.example first, which then forwards to 203.0.113.50',
      'Nowhere: browsers treat a link containing @ as invalid',
    ],
    answer: 'To 203.0.113.50: the part before @ is just a username',
    misconceptions: {
      'To portal.corp.example, the company\'s real login portal': 'phish-url-domain',
    },
    explanation:
