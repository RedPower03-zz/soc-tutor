...
const items = [
  {
    ...
    choices: [
      'The activity the rule describes really happened and it is malicious or unwanted',
      'The rule fired, but the activity it describes did not actually happen',
      'The activity happened, but it was authorised (for example an IT admin at work)',
      'A real attack happened, but no alert fired',
    ],
    answer: 'The activity the rule describes really happened and it is malicious or unwanted',
    misconceptions: { 'The activity happened, but it was authorised (for example an IT admin at work)': 'triage-fp-btp' },
    explanation:
  ...
  },
  {
    ...
    choices: ['Benign true positive', 'False positive', 'True positive', 'False negative'],
  ...
  },
  {
    ...
    choices: [
      'SRV-DC01: priority weighs asset criticality and blast radius, not just the rule\'s severity',
      'LAB-KIOSK-3: it is easier, so it clears the queue faster',
      'Either: they have the same severity, so they have the same priority',
      'Whichever arrived first by a few seconds',
    ],
    answer: 'SRV-DC01: priority weighs asset criticality and blast radius, not just the rule\'s severity',
    misconceptions: {
      'Either: they have the same severity, so they have the same priority': 'triage-severity-priority',
      'Whichever arrived first by a few seconds': 'triage-severity-priority',
    },
    explanation:
...
CommandLine: findstr /i "mimikatz" C:\Reports\edr-detections-sept.csv`,
    choices: [
      'False positive: the keyword matched a text search, no credential tool ran; tune the rule',
      'True positive: mimikatz was executed on SOC-WS-02',
      'Benign true positive: a SOC analyst ran mimikatz as part of her job',
      'Benign: exclude SOC-WS-02 from the rule so it stops firing',
    ],
    answer: 'False positive: the keyword matched a text search, no credential tool ran; tune the rule',
    misconceptions: {
      'Benign: exclude SOC-WS-02 from the rule so it stops firing': 'triage-broad-allowlist',
      'Benign true positive: a SOC analyst ran mimikatz as part of her job': 'triage-fp-btp',
      'True positive: mimikatz was executed on SOC-WS-02': 'triage-raw-event',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Confirm through a separate channel (phone or chat to a known number) and check the sign-in details before closing',
      'Close it as benign: Mark confirmed it',
      'Close it as a false positive: the sign-in was Mark',
      'Disable Mark\'s account permanently',
    ],
    answer: 'Confirm through a separate channel (phone or chat to a known number) and check the sign-in details before closing',
    misconceptions: {
      'Close it as benign: Mark confirmed it': 'triage-blocked-closed',
      'Close it as a false positive: the sign-in was Mark': 'triage-fp-btp',
    },
    explanation:
...
4  Low   New browser extension installed         WS-MKT-11 / leo`,
    choices: [
      '2: an active, repeating connection from a database server with a service account',
      '1: it is High and says malware',
      '3: it is High and an account is locked',
      '4: it is the quickest to close',
    ],
    answer: '2: an active, repeating connection from a database server with a service account',
    misconceptions: {
      '1: it is High and says malware': 'triage-severity-priority',
      '3: it is High and an account is locked': 'triage-severity-priority',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Exclude that signed backup binary, running as the backup service account, on the file servers only',
      'Exclude the file servers from the rule entirely',
      'Disable the rule: it only produces noise',
      'Leave it: analysts can close it every morning',
    ],
    answer: 'Exclude that signed backup binary, running as the backup service account, on the file servers only',
    misconceptions: {
      'Exclude the file servers from the rule entirely': 'triage-broad-allowlist',
      'Disable the rule: it only produces noise': 'triage-broad-allowlist',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Keep working it: find the document and email, check other recipients and look for any stage that was not blocked',
      'Close it: the EDR blocked it, so there is nothing left to do',
      'Close it as a false positive: nothing ran',
      'Reimage WS-FIN-02 immediately',
    ],
    answer: 'Keep working it: find the document and email, check other recipients and look for any stage that was not blocked',
    misconceptions: {
      'Close it: the EDR blocked it, so there is nothing left to do': 'triage-blocked-closed',
      'Close it as a false positive: nothing ran': 'triage-fp-btp',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'What the rule detects and the raw event(s) that triggered it',
      'The user\'s social media profile',
      'Whether the same alert was closed last week, so you can close it the same way',
      'The SIEM\'s licence usage dashboard',
    ],
    answer: 'What the rule detects and the raw event(s) that triggered it',
    misconceptions: { 'Whether the same alert was closed last week, so you can close it the same way': 'triage-raw-event' },
    explanation:
