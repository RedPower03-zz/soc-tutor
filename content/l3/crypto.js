...
const items = [
  {
    ...
    choices: [
      'Nothing: it is Base64 (UTF-16LE) encoding, which anyone can decode without a key',
      'AES-256 encryption',
      'A one-way hash',
      'A digital signature',
    ],
    answer: 'Nothing: it is Base64 (UTF-16LE) encoding, which anyone can decode without a key',
    misconceptions: { 'AES-256 encryption': 'crypto-encoding-is-encryption' },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Symmetric (e.g. AES-GCM or ChaCha20-Poly1305) with a shared key',
      'Asymmetric (RSA) for every byte',
      'A hash function such as SHA-256',
      'Base64',
    ],
    answer: 'Symmetric (e.g. AES-GCM or ChaCha20-Poly1305) with a shared key',
    misconceptions: { 'Asymmetric (RSA) for every byte': 'crypto-symmetric-asymmetric', Base64: 'crypto-encoding-is-encryption' },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'MD5 is fast, so attackers test billions of guesses per second, and without salt identical passwords share a hash and precomputed tables work',
      'MD5 hashes can be decrypted with the MD5 key',
      'MD5 is encryption, not hashing',
      'It is not bad: hashes cannot be reversed',
    ],
    answer: 'MD5 is fast, so attackers test billions of guesses per second, and without salt identical passwords share a hash and precomputed tables work',
    misconceptions: {
      'MD5 hashes can be decrypted with the MD5 key': 'crypto-hash-reversible',
      'It is not bad: hashes cannot be reversed': 'crypto-hash-reversible',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Decryption needs the attacker\'s RSA private key; restore from offline backups and check for a public decryptor (e.g. No More Ransom) in case of an implementation flaw',
      'Extract the public key from the sample and use it to decrypt the files',
      'Brute-force the AES-256 keys over a weekend',
      'Hash the encrypted files to get the originals back',
    ],
    answer: 'Decryption needs the attacker\'s RSA private key; restore from offline backups and check for a public decryptor (e.g. No More Ransom) in case of an implementation flaw',
    misconceptions: {
      'Extract the public key from the sample and use it to decrypt the files': 'crypto-symmetric-asymmetric',
      'Hash the encrypted files to get the originals back': 'crypto-hash-reversible',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'It proves the file was signed by the holder of that private key and has not changed since, not that the signer is trustworthy',
      'It proves the file contains no malware',
      'It encrypts the installer so only you can run it',
      'It is just a SHA-256 hash of the file',
    ],
    answer: 'It proves the file was signed by the holder of that private key and has not changed since, not that the signer is trustworthy',
    misconceptions: { 'It is just a SHA-256 hash of the file': 'crypto-hash-reversible' },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'A later theft of the server\'s private key being used to decrypt previously recorded sessions',
      'Phishing sites with valid certificates',
      'Malware on the client',
      'Weak passwords',
    ],
    answer: 'A later theft of the server\'s private key being used to decrypt previously recorded sessions',
    misconceptions: { 'Phishing sites with valid certificates': 'crypto-symmetric-asymmetric' },
    explanation:
