...
const items = [
  {
    ...
    choices: [
      'A trusted CA bound this key to the names listed',
      'The website is safe to use and free of malware',
      'The company behind the site is honest and legitimate',
      'The connection cannot be monitored by anyone at all',
    ],
    answer: 'A trusted CA bound this key to the names listed',
    misconceptions: {
      'The website is safe to use and free of malware': 'pki-valid-cert-safe',
      'The company behind the site is honest and legitimate': 'pki-valid-cert-safe',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      'The root CA directly, every time',
      'The leaf certificate signs itself',
      'The browser vendor\'s own CA',
    ],
    answer: 'An intermediate CA certificate',
    misconceptions: {
      'The leaf certificate signs itself': 'pki-selfsigned-bad',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Self-signed, days old, ten-year validity: check its context',
      'A normal certificate from a public CA',
      'It must be malicious, because every self-signed certificate is',
      'It has already expired, so browsers will refuse to connect to it',
    ],
    answer: 'Self-signed, days old, ten-year validity: check its context',
    misconceptions: {
      'It must be malicious, because every self-signed certificate is': 'pki-selfsigned-bad',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Reject it: the name must be in the SAN list',
      'Accept it: the CN matches the host name that was typed',
      'Accept it: the issuing CA is trusted by the browser',
      'Reject it: certificates must not be valid until 2027',
    ],
    answer: 'Reject it: the name must be in the SAN list',
    misconceptions: {
      'Accept it: the CN matches the host name that was typed': 'pki-cn-vs-san',
      'Accept it: the issuing CA is trusted by the browser': 'pki-valid-cert-safe',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Any trusted CA can sign for any site, enabling interception',
      'Extra CA certificates slow down every TLS handshake the browser makes',
      'CA:TRUE means the certificate has expired and must be renewed at once',
      'It only matters for S/MIME email',
    ],
    answer: 'Any trusted CA can sign for any site, enabling interception',
    explanation:
  ...
  },
  {
    ...
    snippet:
      'ssl.log  ts=2026-09-24T03:14:07Z id.orig_h=10.20.4.31 id.resp_h=203.0.113.88 id.resp_p=443\n         version=TLSv12 server_name=- validation_status="self signed certificate"\n         ja3=72a589da586844d7f0818ce684948eea ja4=t12i150700_7c1d2a9e5f3b_e4a91b0c6d28\nx509.log certificate.subject="CN=localhost" certificate.issuer="CN=localhost"\n         certificate.not_valid_before=2026-09-22 certificate.not_valid_after=2027-09-22\nconn.log duration=0.8s  repeated every 60s since 2026-09-22T09:00Z',
    choices: [
      'Find the process on 10.20.4.31; pivot on JA3/JA4 and the IP',
      'Ignore it: the traffic is TLS-encrypted, so it is safe by design',
      'Block every self-signed certificate across the whole company now',
      'Ask the issuing CA to revoke the certificate and close the alert',
    ],
    answer: 'Find the process on 10.20.4.31; pivot on JA3/JA4 and the IP',
    misconceptions: {
      'Ignore it: the traffic is TLS-encrypted, so it is safe by design': 'pki-valid-cert-safe',
      'Block every self-signed certificate across the whole company now': 'pki-selfsigned-bad',
    },
    explanation:
      'No SNI, a raw IP, a "CN=localhost" self-signed certificate and a fixed 60-second interval together look like a C2 beacon (many frameworks ship such defaults). The endpoint tells you which process; the client fingerprint and IP find other infected hosts. **JA3** hashes the ClientHello fields in the order sent; since browsers began randomising TLS extension order (Chrome, 2023) a browser\'s JA3 changes between connections, so the newer **JA4** (which sorts ciphers and extensions, and encodes the TLS version, SNI presence and ALPN, e.g. `t12i...` = TCP, TLS 1.2, no SNI) is the more stable pivot. An implant with a fixed TLS library keeps a stable JA3 either way. Nobody can revoke a self-signed certificate: there is no CA.',
  },
  {
    id: 'pki-10',
    skill: 'l3-pki',
  ...
  },
  {
    ...
    choices: [
      'The certificate is encrypted after key exchange; use SNI, JA4',
      'TLS 1.3 servers no longer present certificates to clients at all',
      'TLS 1.3 only runs over UDP, which network sensors cannot capture',
      'Sensors can still read everything; nothing changed in TLS 1.3',
    ],
    answer: 'The certificate is encrypted after key exchange; use SNI, JA4',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Not by itself; the risk is users learning to click through warnings',
      'Yes: self-signed certificates are always a sign of malware on the host',
      'Yes: self-signed certificates provide no encryption for the traffic',
      'No: self-signed certificates are more secure than CA-signed ones',
    ],
    answer: 'Not by itself; the risk is users learning to click through warnings',
    misconceptions: {
      'Yes: self-signed certificates are always a sign of malware on the host': 'pki-selfsigned-bad',
      'Yes: self-signed certificates provide no encryption for the traffic': 'pki-selfsigned-bad',
    },
    explanation:
  ...
  worked: [
    {
      ...
      steps: [
        ...
        'Next: the endpoint (EDR) shows which process opened the socket; pivot on 203.0.113.88 and the JA3/JA4 client fingerprint in the proxy and Zeek logs for other hosts.',
      ],
